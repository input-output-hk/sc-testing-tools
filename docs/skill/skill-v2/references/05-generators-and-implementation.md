...
5. **`initialize` is model-only. Deployment is an Action. Always.**
   Always have a deploy action. The generator emits it when
   uninitialised. No exceptions. The Cardinal Rule at the top of this
   file is the canonical wording; this point is just a reminder.
   Whether the Action type has a single deploy variant or several
   (inline vs hash datum, with/without ref script, …) is a contract
   question — but at least one MUST exist.
...
## 1c. Cardinal-rule anti-pattern (initialize-as-deployer)

The classic violation. Showing it explicitly because it has been
proposed by agents in the past and silently broke deployment testing.

```haskell
-- WRONG: initialize does on-chain work; Action type has no deploy variant
data Action PingPongModel = DoPing | DoPong | DoStop

initialize = do
  utxo <- deployScript  -- WRONG: this is on-chain work
  pure PingPongModel{ modelState = Pinged, modelUtxo = utxo }
```

Why this is wrong:

- `initialize` submits a tx (`deployScript`). The Cardinal Rule
  forbids that.
- The Action type has no `Start` / `Deploy` variant. Deployment can
  never be exercised by the generator — every round starts from a
  hard-coded post-deploy state, so deployment itself is never tested.
- Threat models can't probe the deploy tx either; it didn't go
  through the framework.

The right version:

```haskell
-- RIGHT: deploy is an action; initialize is model-only
data Action PingPongModel = Start | DoPing | DoPong | DoStop

initialize = pure PingPongModel
  { modelState       = Pinged
  , modelUtxo        = Nothing
...
arbitraryAction PingPongModel{modelInitialized} =
  if not modelInitialized
    then pure Start
    else QC.elements [DoPing, DoPong, DoStop]
```

Now: `initialize` only sets up the model. The first action generated
in any round is `Start`, which has its own `perform` branch doing the
on-chain deploy. Subsequent actions are normal vocabulary. Deployment
is now under test like every other action.
...
-- modelInitialized = False (or equivalent). DOES NOT submit any tx,
-- DOES NOT deploy, DOES NOT touch the chain. Deployment is an
-- Action emitted by the generator when the model is uninitialised.
```

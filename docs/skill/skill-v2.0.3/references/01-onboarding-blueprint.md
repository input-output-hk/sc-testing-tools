...
1. **`initialize` runs ONCE** at the start of the round. It sets up
   the in-memory model state and returns it. Whether it also performs
   on-chain deployment depends on the contract (see §E.1 below).
...
### E.1 Where deployment lives: two valid shapes

There is **no universal rule** that `initialize` must deploy the
contract. Pick based on what you want to test:

- **Simple case — one deployment shape.** If the contract has only
  one way to be deployed and nothing about deployment is worth
  exercising, `initialize` may perform the deploy and return a
  post-bootstrap model. Action vocabulary is steady-state only
  (e.g. `{DoPing, DoPong, DoStop}`). No `Start` / `Deploy` action.
- **Rich case — multiple deployment shapes.** If the contract can be
  deployed in distinct ways worth testing (inline datum vs hash
  datum, with vs without reference script, single vs multiple script
  UTxOs), `initialize` only sets up the **model** with
  `initialised = False` and returns. Deployment becomes an
  **action**: e.g. `StartWithInlineDatum`, `StartWithDatumHash`.
  The generator emits a `Start*` action when the model says we're
  uninitialised.

Both shapes are correct. The canonical `PingPongSpec.hs` example in
`convex-testing-interface` uses the rich case — its model has
`modelInitialized :: Bool` and the `Action` type includes
`StartWithInlineDatum | StartWithDatumHash` precisely because
testing both deployment styles is in scope.

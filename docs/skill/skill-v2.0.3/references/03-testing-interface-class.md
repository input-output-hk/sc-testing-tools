...
Runs once at the start of every QuickCheck iteration. Deploy reference
scripts, pay setup UTxOs, mint initial tokens, set initial datums.
Returns the initial model state.

The body usually calls the user's off-chain helpers via
`tryBalanceAndSubmit` (see `04-helpers-cheatsheet.md §A`). If the
contract needs no on-chain setup, the body is just
`pure (MyModel ...)`.

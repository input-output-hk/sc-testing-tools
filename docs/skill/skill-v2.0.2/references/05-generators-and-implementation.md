...
**semantic** validity ("can the contract accept this right now?")
instead of **structural** validity ("can I even construct this action
from the model data?"). Get this wrong and the negative channel
silently dies.

**Rule:**

- **Structural branching belongs in `arbitraryAction`.** Branch only
  when the action literally cannot be *constructed* from the available
  model data — e.g. there is no script UTxO yet to reference, or there
  is no entry in some required map.
- **Semantic branching belongs in `precondition`.** When the action is
  fully constructible and well-formed but the contract should reject
  it given current state (e.g. "ping while already pinged"), the
  generator must produce it and `precondition` must say `False`.
...
**Heuristic.** Ask yourself: *"Can I construct this action from the
data I have? If yes, generate freely. If no, branch."* Anything else
...
arbitraryAction s = case msThread s of
    Nothing       -> pure Seed
    Just Pinged   -> oneof [pure DoPong, pure DoStop, pure MalformedRedeemer]
    Just Ponged   -> oneof [pure DoPing, pure DoStop, pure MalformedRedeemer]
    Just Stopped  -> oneof [pure DoPing, pure DoPong, pure DoStop, pure MalformedRedeemer]
```

Two errors here:

1. `DoPing`, `DoPong`, `DoStop` are all fully constructible — they need
   nothing from the model that isn't always there. Branching by
   `msThread` is **semantic**, not structural. The framework now never
   sees `DoPing` while `msThread = Just Pinged`, so it never verifies
   the contract rejects that case. Negative channel dies.
2. `MalformedRedeemer` is a **structural attack**, not a contract
   operation. It does not belong in the action vocabulary. See §1b.

### Corrected

```haskell
arbitraryAction _s = oneof [pure DoPing, pure DoPong, pure DoStop]

precondition s DoPing = msThread s == Just Ponged || msThread s == Just Stopped
precondition s DoPong = msThread s == Just Pinged
precondition s DoStop = isJust (msThread s)
```

(Note: `Seed` is gone too. Bootstrap belongs in `initialize`; see
`references/01-onboarding-blueprint.md §E`.)
...
```haskell
-- Bootstraps the contract for a single test round: deploys the validator,
-- funds participants, and returns the model state that represents
-- "system is ready". Runs ONCE per trace; subsequent actions assume
...
```haskell
-- Proposes the next Action. Intentionally "dumb" about validity --
-- only branches on STRUCTURAL shape (what data can I construct from
-- the model?), never on SEMANTIC validity (is this action allowed
-- right now?). Semantic gating is precondition's job. Keeping the
-- generator dumb is what feeds the negative-testing channel.
```

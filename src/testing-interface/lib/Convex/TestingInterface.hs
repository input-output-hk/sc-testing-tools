...
class (TestingInterface state) => ThreatModelsFor state where
  ...
  Overriding this list makes it a coverage claim: an explicitly listed model
  whose precondition never holds on any generated transaction fails the
  suite (see 'threatModelTestCase'), so list only models that actually apply
  to the contract's transaction shapes. The default list carries no such
  claim - it means "run whatever applies" and tolerates models that never
  do.
  -}
  threatModels :: [ThreatModel ()]
  threatModels = defaultThreatModelsExcluding (expectedVulnerabilities @state <> acceptedFindings @state)
...
propRunActionsWithOptions groupName opts =
  withFrozenCallStack $
    withSrcLoc $
      askOption $ \(recorder :: TraceRecorder) ->
        let tms = nameFallbacks "Threat model" (threatModels @state)
            filteredTms = filterThreatModelsByOptions opts tms
            evs = nameFallbacks "Expected vulnerability" (expectedVulnerabilities @state)
            afs = nameFallbacks "Accepted finding" (acceptedFindings @state)
            -- An explicitly overridden 'threatModels' list is a coverage
            -- claim, so a listed model that never applies fails its test
            -- case; the default list means "run whatever applies" and
            -- tolerates that (see 'defaultThreatModelsExcluding').
            usesDefaultTms =
 ...
 where
  ...
  threatModelGroup usesDefaultTms getTmResultsRef tms' =
    [testGroup "Threat models" $ zipWith (threatModelTestCase usesDefaultTms getTmResultsRef "Threat models") [1 ..] tms']
...
threatModelTestCase
  ...
  -- ^ The threat model
  -> TestTree
threatModelTestCase usesDefaultTms getTmResultsRef groupName idx tm =
                    ...
                    then
                      ...
                      step $
                        "SKIPPED: Precondition never met ("
                          <> show numSkipped
                          <> " precondition"
                          <> ( if numSkippedPhase1 > 0
                                 then ", " <> show numSkippedPhase1 <> " phase 1 invalidation / rebalancing failure"
                                 else ""
                             )
                          <> ", 0/"
                          <> show total

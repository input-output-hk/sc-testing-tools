...
expandSelectedTestIds testMap selectedIds =
 ...
 where
  ...
  -- Every per-model group, whatever its category, needs the positive tests
  -- that populate its outcomes: selecting one alone would report
  -- "No tests were generated by positive tests".
  perModelGroups =
    Set.fromList (map (Text.pack . threatModelGroupName) [minBound .. maxBound :: ThreatModelCategory])
